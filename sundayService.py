...
#
# write my PID into a local file
#
myPID = str(os.getpid())
...
def checkSoundRecordingPrio():
    ...
    if not schTaskIsRunning("SundayServiceRec"):
        print("SoundRecorder was not running!" )
        cmd = "schtasks.exe /run /tn SundayServiceRec" 
        result = (subprocess.check_output(cmd)).decode('utf-8').splitlines()
        Logger.debug( str(result) )
    else:
        print( "Soundrecorder was running..." )
        
    # set process priority to high
    for pid in getPID('soundrecorder'):
        p = psutil.Process( int( pid ) )
        print('prio of soundrecorder', p, 'is', p.nice)
        if p.nice() < 128:
            p.nice( psutil.HIGH_PRIORITY_CLASS )
            print('prio changed to ', psutil.HIGH_PRIORITY_CLASS)
...
def checkSoundRecordingFile():
    ...
    if os.path.isfile( soundRecordingFile ):
        print('****', soundRecordingFile + " found! ****" )
        path = os.path.dirname(soundRecordingFile)
        newName = 'sunday '+str(now.year)+'-'+str(now.month).rjust(2,'0')+'-'+str(now.day).rjust(2,'0')+'.wma'
        newName = os.path.join(path, newName)
        print('renaming to', newName)
        os.replace( soundRecordingFile, newName )
        soundRecordingRenamed = True
        # try copying file to Dropbox area
        print("Trying to copy " + newName + " to " + soundRecordingFileUploadPath)
        try:
            shutil.copy2(newName, soundRecordingFileUploadPath)
        except:
            Logger.exception("copy of "+ newName + ' to ' + soundRecordingFileUploadPath + 'failed!')
            print("copy of "+ newName + 'failed!')
    else: 
        print('File not found: '+soundRecordingFile)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if now.minute % 5 == 0     or count == 0:
            ...
            if onWindows: 
                print('checkForRecordingFile')
                checkForRecordingFile()
                print('checkForNewsletterFile')
        ...
        #---------------------------------------------------------------------------------------------------
        # end the script before midnight
